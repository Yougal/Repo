...
public class SnakeAndLadder {
	public static void main(String[] args) {
		...
		int N = 30;
		int[] moves = constructGame(N);
		
		System.out.println(getMinDiceThrows(moves, 0));
	}

	private static int[] constructGame(int N) {
		int moves[] = new int[N];
    ...
    static int getMinDiceThrowsEff(int move[], int n) 
    {
	...
	private static int getMinDiceThrows(int[] moves, int n) {
		...
		for(int i = 1;i<6;i++){
			...
			moves[n]=-2;
			if(value==-1){
				int minDiceThrows = getMinDiceThrows(moves, n+i);
				if(minDiceThrows!=Integer.MAX_VALUE){
					currMin = 1 + minDiceThrows;
				}
			}else{
				int minDiceThrows = getMinDiceThrows(moves,  value);
				if(minDiceThrows!=Integer.MAX_VALUE){
					currMin = 1 + minDiceThrows;
				}
			}
